...
public class BoardDAO extends JDBConnection{
	...
	public List<Board> list() {
		...
		try {
			...
			while(rs.next()) {		// next() : 실행 결과의 다음 데이터로 이동
				...
				// 결과 데이터 가져오기
				// rs.getXXXX("컬럼명") --> 해당 컬럼의 데이터를 가져온다
				// * "컬럼명"의 값을 특정 타입으로 변환
	...
	public int insert(Board board) {
		...
		try {
			...
			result = psmt.executeUpdate();			// SQL 실행 요청, 적용된 데이터 갯수를 받아온다.
													// 게시글 1개 적용 성공 시, result : 1
													//			   실패 시, result : 0
			// executeUpdate()
			// : SQL (INSERT, UPDATE, DELETE)을 실행하고 적용된 데이터 갯수를 int 타입으로 반환
		} catch (SQLException e) {
