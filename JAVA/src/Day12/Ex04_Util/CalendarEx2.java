...
public class CalendarEx2 {
	...
	public static void main(String[] args) {
		...
		// 추첨일 오후 8시 부터 다음일(일요일) 오전 6시 까지는 판매 정지
		...
		// 로또 구입이 불가한 조건
		// 1.(구입 불가) 오전 00시 ~ 06시
		if(Am_Pm == 0 && Time >= 0 && Time <= 6) {
			System.out.println("구입 가능한 시간이 아닙니다.");
		}
		
		// 2.(구입 불가) 토요일이면서, 오후 8시 이후
		else if(todayOfWeek.equals("토") && Am_Pm == 1 && Time >= 8) {
			System.out.println("구입 가능한 시간이 아닙니다.");
		}
		// 3.(구입 가능)
		else {
		...
		if(buyYn) {
			// 추첨일 구하는 계산
			lottoCal.add(Calendar.DATE, 7 - Day);
			String prizeDay = sdf.format(lottoCal.getTime());
			System.out.println("추첨일 : " + prizeDay);
		} else {
